...
const Skills: React.FC<SkillsProps> = ({ skills }) => {
    ...
    const isLastOdd = categories.length % 2 !== 0;
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
                {categories.map((category, index) => (
                    <div
                        key={category.name}
                        className={`glass p-6 rounded-3xl border border-slate-200/50 dark:border-slate-850/50 flex flex-col hover:border-primary/20 transition-all duration-300 shadow-sm hover:shadow-md${isLastOdd && index === categories.length - 1 ? ' md:col-span-2' : ''}`}
                    >
